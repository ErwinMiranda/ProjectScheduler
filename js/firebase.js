...
export async function addTask(wo, acreg, title, start, end, dep) {
  ...
  return addDoc(tasksCol, {
    ...
    // OPTIONAL: initialize revised dates
    revstart: startISO,
    revend: endISO,

    status: "open",
    actual_finish_key: null,
    depends: dep || [],
    dailyStatus: {},
    row: Date.now(),
    lagDays: 0,
...
export async function saveTask(task) {
  if (!task || !task.id) {
    throw new Error("saveTask: task or task.id missing");
  }
  ...
  // ✅ PRESERVE per-day milestone progress
  if (task.dailyStatus) {
    payload.dailyStatus = task.dailyStatus;
  }
  ...
  return onSnapshot(q, (snap) => {
    const list = snap.docs.map((d) => {
      ...
      return {
        ...
        actual_finish_key: Number.isFinite(data.actual_finish_key)
          ? data.actual_finish_key
          : null,
...
export async function batchSaveTasks(taskArray) {
  ...
  for (let i = 0; i < taskArray.length; i++) {
    ...
    const payload = {
      ...
      actual_finish_key: Number.isFinite(t.actual_finish_key)
        ? t.actual_finish_key
        : null,
...
export async function closeTaskForDay(taskId, dateKey) {
  const ref = doc(db, "tasks", taskId);
  await updateDoc(ref, {
    [`dailyStatus.${dateKey}`]: "Closed",
    updatedAt: serverTimestamp(),
  });
}
