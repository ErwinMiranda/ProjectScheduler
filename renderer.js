...
function buildRows(taskLeftList, rowsRight, minDate, width) {
  ...
  tasks.forEach((task, idx) => {
    ...
    bar.addEventListener("mousemove", () => {
      ...
      tooltip.innerHTML = `
        ...
        <div style="font-size:12px; color:#334155">
          ...
          Type: FS (Finish→Start)<br/>
          Lead: ${lead} day(s)<br/>
          Lag: ${lag} day(s)<br/>
          Net offset: ${offset >= 0 ? "+" : ""}${offset} day(s)
        </div>
      `;
    ...
    bar.addEventListener("contextmenu", (e) => {
      ...
      // hide tooltip while menu opens
      tooltip.style.opacity = "0";

      const current = Number.isFinite(t.lagDays) ? t.lagDays : 0;
      const val = prompt("Set lag days (0 = none):", String(current));
      if (val === null) return;

      const lag = Math.max(0, parseInt(val) || 0);
      t.lagDays = lag;

      // local only — signal unsaved change and refresh UI
      window.dispatchEvent(new CustomEvent("localchange"));

      applyDependencies();
      render();

      // refresh dropdown (local)
